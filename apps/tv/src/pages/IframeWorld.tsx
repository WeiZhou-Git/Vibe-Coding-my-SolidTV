...
const getTemperatureValue = (temperature: string) => temperature.replace("℃", "°");
...
const getCenteredTextWidth = (text: string) => Math.min(880, Math.max(420, text.length * 30));
...
const IframeWorld = () => {
  ...
  const [selectedForecastTime, setSelectedForecastTime] = createSignal(
    getDefaultForecastTime(fallbackForecast),
  );
  ...
  onMount(() => {
    ...
    spinnerFrame = requestAnimationFrame(updateSpinner);
    iframe = createRainDemoIframe(
      () => setLoading(false),
      getRainDemoHash(currentForecast()),
      getRainTextureGroup(),
    );
    ...
    fetchRealForecast7dWeather(weatherAbortController.signal)
      .then(weatherForecast => {
        ...
        const defaultForecast =
          weatherForecast.find(item => item.forecasttime === defaultForecastTime) ||
          getCurrentForecast(weatherForecast);
  ...
  return (
    <View style={PageStyle}>
      ...
      <View
        ...
        transition={MainContentTransition}
      >
        <Text style={TemperatureValueStyle}>{getTemperatureValue(currentForecast().temperature_am)}</Text>
