...
const Home = () => {
  ...
  let atmosphereRow: ElementNode | undefined;
  let wallpaperFilterRow: ElementNode | undefined;
  let wallpaperGrid: ElementNode | undefined;
  let profileActionRow: ElementNode | undefined;
  ...
  const renderTabContent = () => {
    switch (activeNavIndex()) {
      ...
      case 3:
        return (
          <ProfileTabContent
            onActionUp={() => navRow?.setFocus()}
            onActionReady={row => {
              profileActionRow = row;
            }}
          />
        );
      default:
          ...
          {navItems.map((item, index) => (
            <NavItem
              ...
              onDown={() => {
                ...
                if (index === 1) {
                  atmosphereRow?.setFocus();
                  return;
                }

                if (index === 2) {
                  wallpaperFilterRow?.setFocus();
                  return;
                }

                if (index === 3) {
                  profileActionRow?.setFocus();
                  return;
                }
              }}
